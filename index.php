	...
	<div class="container-fluid">
      <div class="row">
        ...
        <?php
			...
			function MemberLoadForm($selected){
				echo '<script type="text/javascript">panelIndex=2;</script>';
				echo '<h2 class="sub-header">Update member progress</h2><br>';
				echo '<form action="." method="get">';
				echo '<table width="100%" border="0" ><tr><td width="15%" ><div class="extra">';
				echo '<input type="hidden" class="form-control" value="ProgressLoaded" name="page">';						
				echo '<label for="member">Member : </label></div>';
				echo '</td><td td width="75%">';				
				echo '<select class="form-control" id="member" name="member">'.remainingMemberDropDown($selected).'</select>';
				echo '</td><td width="10%"><div class="extra"><button type="submit" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-ok-circle"></span> View Pending Projects</button></div>';	
			...
			function remainingMemberDropDown($selected){
				global $con; //Refer to the global variables
				$sql="SELECT m.`Index`,m.`Name` FROM `toastmastersdb`.`tt_members` AS m WHERE m.`Index`<>".$_SESSION['Index'];
				$options="";
